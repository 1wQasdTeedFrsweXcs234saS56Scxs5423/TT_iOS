...
- (CDVPluginResult*)processVideo:(NSString*)moviePath forCallbackId:(NSString*)callbackId
{
    ...
    // create MediaFile object
//    NSString *locationPath = @"Documents/Contribute/";
    NSLog(@"This is my movie Path %@", moviePath);
    
    NSRange range = [moviePath rangeOfString:@"tmp"];
...
- (void)viewDidLoad
{
    ...
    int i = 1;
    do {
        NSString* docsPathTemp = [NSTemporaryDirectory ()stringByStandardizingPath];
        NSString *locationPath = @"Documents/Contribute";
        NSLog(@"This is my Docs Save Path %@", docsPath);
        docsPath = [docsPathTemp substringToIndex:[docsPathTemp length] - 3];
        docsPath = [NSString stringWithFormat:@"%@%@", docsPath, locationPath];
        filePath = [NSString stringWithFormat:@"%@/audio_%03d.wav", docsPath, i++];
        NSLog(@"This is my Audio Save Path %@", docsPath);
        // WARNING: Executing in Reverse order
    } while ([fileMgr fileExistsAtPath:filePath]);
