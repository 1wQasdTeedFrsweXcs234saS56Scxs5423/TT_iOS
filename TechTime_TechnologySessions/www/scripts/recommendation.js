...
function loadrecoEngineData(){
    var loggedInUsername = jsonData.loggedUserName.replace(/_/g, '.');
    var recoEngineRSS = "https://techtime.accenture.com/mobile-recommendation/"+loggedInUsername;
       // var recoEngineRSS = "https://techtime.accenture.com/mobile-recommendation/k.veni.ragalapally";
...
function displayRecoEngine(){
    ...
    $.each(jsonData.recommendations, function(key, item) {
           itemit.push(item);
    ...
    if(whatLeadersAreWatchingRecommendations.length != 0)
    {
        $.each(whatLeadersAreWatchingRecommendations, function(key, item){
               ...
               strHTMLtext = strHTMLtext + '<span style="display: inline-block;width:80%;" onclick=recoEngineDetailPageView("' + item.cid + '","' + item.ctype + '","' + count + '");><span style="font-style:italic;"><b>' + item.viewer + '</b></span> <span> viewed</span></br><span>\"' + item.title + '\"</span></span>';
    ...
    $.each(jsonData.recommendations, function(key, item) {
           ...
           strHTMLtext = strHTMLtext + "<table border=0 style='width:100%;padding:0px;margin-top:5px;margin-bottom:5px;border:none;margin-right:2px;margin-left:5px;' cellpadding='0' cellspacing='0' onclick=recoEngineDetailPageView('"+item.cid+"','"+item.ctype+"','"+count+"');>";
    ...
    $.each(jsonData.recommendations, function(key, item) {
           ...
           strHTMLtext = strHTMLtext + "<table border=0 style='width:100%;margin:0px;padding:0px;margin-top: 5px;margin-bottom: 5px;border:none;margin-right: 2px;margin-left: 5px;' cellpadding='0' cellspacing='0' onclick=recoEngineDetailPageView('" + item.cid + "','" + item.ctype + "','" + count + "');>";
    ...
    $.each(jsonData.recommendations, function(key, item) {
           ...
           strHTMLtext = strHTMLtext + "<table border=0 data-cid='"+item.cid+"' data-ctype='"+item.ctype+"' style='width:100%;margin:0px;padding:0px;margin-top: 5px;margin-bottom: 5px;margin-left: 5px;border:none;margin-right: 2px;' cellpadding='0' cellspacing='0' onclick=recoEngineDetailPageView('" + item.cid + "','" + item.ctype + "','" + count + "');>";
...
function recoEngineDetailPageView(elementId, type, countNum)
{
